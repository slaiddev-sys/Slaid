...
export interface ProductiPhoneShowcaseProps {
  /**
  ...
  productImages?: {
    ...
    /**
     * Main product mockup image (iPhone in hand, etc.)
     */
...
/**
 * Product iPhone Showcase Layout
 * 
 * A specialized layout for showcasing mobile products with explanatory text on the left 
 * and iPhone mockup in hand on the right. Features background image and iPhone mockup 
 * for mobile app and product presentation.
 */
export default function Product_iPhoneShowcase({
  title = 'Our Solution',
  ...
  const containerClasses = useFixedDimensions 
    ? `product-iphone-showcase pl-6 lg:pl-12 pr-0 pt-8 lg:pt-12 pb-6 lg:pb-8 bg-white ${className}`
    : `product-iphone-showcase pl-6 lg:pl-12 pr-0 pt-8 lg:pt-12 pb-6 lg:pb-8 bg-white w-full h-full min-h-[400px] ${className}`;
    ...
    >
      ...
      <div className="flex h-full relative">
        ...
        >
              ...
              {/* iPhone Frame Overlay - Above UI with Figma-style Selection */}
              <div 
                className="absolute inset-0 pointer-events-none"
                style={{ zIndex: 2 }}
        >
                <FigmaImage
                  src={productImages?.mockup?.src || "/Hand-iPhone-MockUp.png"}
                  alt={productImages?.mockup?.alt || "iPhone in hand mockup"}
                  size="full"
                  fit="contain"
                  align="center"
                  rounded={false}
