...
DO NOT provide presentation structure - that will come from the Word document.`;
                ...
                const excelAnalysisResponse = await anthropicWrapper.createMessage({
                  model: 'claude-opus-4-20250514',
                  max_tokens: 4000,
                  messages: [{ role: 'user', content: excelAnalysisPrompt }],
                  kind: 'new',
                  userAction: 'analyze-excel',
                  requestId: `excel-analysis-${Date.now()}`
                });

                const excelAnalysis = excelAnalysisResponse.content;
...
Be specific and accurate - use the comprehensive statistics provided.`;
          ...
          const analysisResponse = await anthropicWrapper.createMessage({
            model: 'claude-opus-4-20250514',
            max_tokens: 4000,
            messages: [
              {
            ...
            kind: 'new',
            userAction: 'analyze-excel-only',
            requestId: `excel-only-analysis-${Date.now()}`
          });
